...
export class UserCalendarComponent implements OnInit {
  ...
  submitAddEvent(): void {
    this.eventsService.createEvent(this.newEvent).subscribe(created => {
      const ops = this.selectedDogs.map(d => this.eventsService.addDogToEvent(created.id, d.id));
      if (ops.length === 0) {
        this.loadEvents();
  ...
  addDogToEvent(dog: Dog): void {
    if (!this.selectedEvent) return;
    this.eventsService.addDogToEvent(this.selectedEvent.id, dog.id).subscribe(updated => {
      this.replaceEvent(updated);
      this.selectedEvent = updated;
      this.popupDogSearch = '';
      this.popupFilteredDogs = [];
